  ...
  async mount(view, repoUrl) {
    ...
    try {
      const [assets, repos, audit] = await Promise.all([
        ...
        // History degrades to empty rather than failing the view — the
        // scoped-asset list still renders.
        this.cached(`audit:${HISTORY_DAYS}`, () => this.sx.usage.auditEvents(HISTORY_DAYS)).catch(() => []),
      ]);
      if (disposed) return;
